...
var require_iterate = __commonJS({
  "node_modules/asynckit/lib/iterate.js"(exports, module) {
    ...
    function runJob(iterator2, key, item2, callback) {
      var aborter;
      if (iterator2.length == 2) {
        aborter = iterator2(item2, async(callback));
      } else {
        aborter = iterator2(item2, key, async(callback));
      }
      return aborter;
    }
  }
...
function sanitizeValue(value, invalidChars) {
  if (utils_default.isArray(value)) {
    return value.map((item2) => sanitizeValue(item2, invalidChars));
  }
...
function toFormData(obj, formData, options) {
  ...
  function build(value, path, depth = 0) {
    ...
    utils_default.forEach(value, function each(el2, key) {
      const result = !(utils_default.isUndefined(el2) || el2 === null) && visitor.call(formData, el2, utils_default.isString(key) ? key.trim() : key, path, exposedHelpers);
      if (result === true) {
...
function isAccountDisabled(message) {
  return typeof message === "string" && message.toLowerCase().includes("d\xE9sactiv\xE9");
}
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && error.response?.data?.expired && !originalRequest._retry) {
      ...
      if (refreshToken) {
        try {
          const { data } = await axios_default.post("/api/auth/refresh-token", { refreshToken });
    ...
    const message = error.response?.data?.error || error.message || "Erreur r\xE9seau";
...
// src/api/hooks.js
var useJobs = (filters = {}, options = {}) => useQuery({
  ...
  queryFn: async () => {
    ...
    const { data } = await axios_default2.get(`/jobs?${params}`);
    return data;
  },
  ...options
});
var useRunScraping = () => {
  ...
  return useMutation({
    mutationFn: async (params) => {
      const { data } = await axios_default2.post("/scraping/run", params || {});
      return data;
    },
    onSuccess: () => {
      qc2.invalidateQueries({ queryKey: ["jobs"] });
      qc2.invalidateQueries({ queryKey: ["scraping", "logs"] });
    }
  });
};
...
var SITE_SOURCES = ["linkedin", "indeed", "welcometothejungle", "rekrute", "manpower", "dreamjob", "emplois"];
var SOURCE_LABELS = {
  linkedin: "LinkedIn",
  indeed: "Indeed",
...
function parseDescription(job) {
  ...
  const parts = {};
  const loc = desc.match(/Localisation:\s*([^\n]+)/i);
  if (loc) parts.location = loc[1].trim();
  const comp = desc.match(/Entreprise:\s*([^\n]+)/i);
  if (comp) parts.company = comp[1].trim();
  const type = desc.match(/Type de contrat:\s*([^\n]+)/i);
  if (type) parts.jobType = type[1].trim();
  const exp = desc.match(/Exp̩rience requise:\s*([^\n]+)/i);
  if (exp) parts.experience = exp[1].trim();
  return parts;
}
function SitesPage() {
  const [search, setSearch] = useState2("");
  const apiFilters = useMemo(() => {
    const filters = { group: "sites", sort: "date", limit: 100 };
    if (search.trim()) filters.search = search.trim();
    return filters;
  }, [search]);
  const { data, isLoading, error, refetch } = useJobs(apiFilters);
  const runScraping = useRunScraping();
  const sites = data?.jobs ?? [];
  const handleScraping = () => runScraping.mutate(
    {},
    {
      onSuccess: (res) => {
        const count = res?.jobsFound ?? res?.offersFound ?? res?.totalFound ?? 0;
        toast.success(`${count} offres collect\xE9es`, { duration: 4e3 });
        refetch();
      },
      onError: (err) => toast.error(err?.response?.data?.error || err?.message || "Erreur lors de la collecte")
    }
  );
  return /* @__PURE__ */ jsx("div", { className: "mx-auto w-full max-w-7xl px-4 py-6", children: /* @__PURE__ */ jsxs(motion.div, { variants: container, initial: "hidden", animate: "show", className: "space-y-6", children: [
    /* @__PURE__ */ jsxs(motion.div, { variants: item, className: "flex flex-col gap-4 md:flex-row md:items-center md:justify-between", children: [
      /* @__PURE__ */ jsxs("div", { className: "space-y-1", children: [
        /* @__PURE__ */ jsxs("h1", { className: "flex items-center gap-2.5 text-3xl font-semibold tracking-tight", children: [
          /* @__PURE__ */ jsx(Globe2, { className: "size-7 text-primary" }),
          "Sites d'emploi"
        ] }),
        /* @__PURE__ */ jsx("p", { className: "text-muted-foreground", children: "Offres collect\xE9es depuis les sites d'emploi marocains et internationaux" })
      ] }),
        ...
        /* @__PURE__ */ jsxs("div", { className: "relative", children: [
          ...
          /* @__PURE__ */ jsx(
            "input",
            {
              ...
              className: "h-9 w-full rounded-md border border-input bg-transparent pl-9 pr-3 text-sm shadow-sm transition-colors placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring md:w-64"
            }
          )
        ] }),
        /* @__PURE__ */ jsxs(
          "button",
          {
            ...
            className: "inline-flex h-9 items-center gap-2 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground shadow-sm transition-colors hover:bg-primary/90 disabled:opacity-50",
            children: [
              runScraping.isPending ? /* @__PURE__ */ jsx(Loader2, { className: "size-4 animate-spin" }) : /* @__PURE__ */ jsx(RefreshCw, { className: "size-4" }),
              runScraping.isPending ? "Collecte..." : "Collecter les offres"
            ]
          }
        )
      ] })
    ] }),
    /* @__PURE__ */ jsx(motion.div, { variants: item, className: "flex flex-wrap gap-1.5", children: SITE_SOURCES.map((id) => /* @__PURE__ */ jsxs(
      "span",
      {
        className: "inline-flex items-center gap-1.5 rounded-full border bg-muted/50 px-2.5 py-1 text-xs",
        children: [
          /* @__PURE__ */ jsx(Building2, { className: "size-3 text-muted-foreground" }),
          SOURCE_LABELS[id] || id
        ]
      },
      id
    )) }),
    isLoading ? /* @__PURE__ */ jsx("div", { className: "space-y-3", children: Array.from({ length: 5 }).map((_, i) => /* @__PURE__ */ jsxs(
      motion.div,
      {
        variants: item,
        ...
        children: [
          /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
            /* @__PURE__ */ jsx("div", { className: "h-4 w-48 animate-pulse rounded bg-muted" }),
            /* @__PURE__ */ jsx("div", { className: "h-3 w-32 animate-pulse rounded bg-muted" })
          ] }),
          /* @__PURE__ */ jsx("div", { className: "h-8 w-24 animate-pulse rounded-md bg-muted" })
        ]
      },
      i
    )) }) : error ? /* @__PURE__ */ jsxs("div", { className: "rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center", children: [
      /* @__PURE__ */ jsx(SearchX, { className: "mx-auto size-10 text-destructive" }),
      /* @__PURE__ */ jsx("p", { className: "mt-3 text-muted-foreground", children: "Une erreur est survenue lors du chargement des offres." }),
      /* @__PURE__ */ jsxs(
        "button",
        {
          onClick: () => refetch(),
          className: "mt-4 inline-flex items-center gap-2 rounded-md border bg-background px-4 py-2 text-sm transition-colors hover:bg-accent",
          children: [
    ...
    ] }) : sites.length === 0 ? /* @__PURE__ */ jsxs(motion.div, { variants: item, className: "rounded-xl border border-dashed bg-muted/30 p-8 text-center", children: [
      /* @__PURE__ */ jsx(Globe2, { className: "mx-auto size-12 text-muted-foreground/60" }),
      /* @__PURE__ */ jsx("h2", { className: "mt-4 text-base font-semibold", children: "Aucune offre collect\xC3\xA9e" }),
      /* @__PURE__ */ jsx("p", { className: "mt-1 text-sm text-muted-foreground", children: "Lancez la collecte des sites d'emploi externes pour afficher les offres ici." }),
      /* @__PURE__ */ jsxs(
        "button",
        {
          onClick: handleScraping,
          disabled: runScraping.isPending,
          className: "mt-5 inline-flex items-center gap-2 rounded-md bg-primary px-5 py-2 text-sm font-medium text-primary-foreground shadow-sm transition-colors hover:bg-primary/90 disabled:opacity-50",
          children: [
            runScraping.isPending ? /* @__PURE__ */ jsx(Loader2, { className: "size-4 animate-spin" }) : /* @__PURE__ */ jsx(RefreshCw, { className: "size-4" }),
            runScraping.isPending ? "Collecte en cours..." : "Lancer la collecte"
          ]
        }
      )
    ] }) : /* @__PURE__ */ jsxs(
      motion.div,
      {
        ...
        children: [
          /* @__PURE__ */ jsxs("p", { className: "mb-3 text-sm text-muted-foreground", children: [
            ...
            " collect\xE9e",
            sites.length > 1 ? "s" : "",
            " depuis les sites d'emploi"
          ] }),
          /* @__PURE__ */ jsx("div", { className: "space-y-3", children: sites.map((job) => {
            const details = parseDescription(job);
            return /* @__PURE__ */ jsxs(
              motion.div,
              {
                variants: item,
                className: "group flex flex-col gap-3 rounded-xl border bg-card p-4 transition-colors hover:border-primary/30 md:flex-row md:items-center md:justify-between",
                children: [
                  /* @__PURE__ */ jsxs("div", { className: "min-w-0 flex-1", children: [
                    /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
                      /* @__PURE__ */ jsx(Briefcase, { className: "size-4 shrink-0 text-primary" }),
                      /* @__PURE__ */ jsx("h3", { className: "truncate font-medium", children: job.title })
                    ] }),
                    /* @__PURE__ */ jsx("p", { className: "mt-1 line-clamp-2 text-sm text-muted-foreground", children: job.description }),
                    /* @__PURE__ */ jsxs("div", { className: "mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground", children: [
                      /* @__PURE__ */ jsxs("span", { className: "inline-flex items-center gap-1", children: [
                        ...
                        job.location || details.location || "Maroc"
                      ] }),
                      /* @__PURE__ */ jsxs("span", { className: "inline-flex items-center gap-1", children: [
                        /* @__PURE__ */ jsx(Building2, { className: "size-3.5" }),
                        job.company || details.company || job.site_label || "Entreprise"
                      ] }),
                      /* @__PURE__ */ jsxs("span", { className: "inline-flex items-center gap-1", children: [
                        /* @__PURE__ */ jsx(Clock, { className: "size-3.5" }),
                        /* @__PURE__ */ jsx("span", { className: "capitalize", children: job.site_source || "site" })
                      ] }),
                      formatDeadline(job.deadline || details.deadline) && /* @__PURE__ */ jsxs("span", { className: "inline-flex items-center gap-1", children: [
                        /* @__PURE__ */ jsx(CalendarClock, { className: "size-3.5" }),
                        formatDeadline(job.deadline || details.deadline)
                      ] })
                    ] })
                  ] }),
                  /* @__PURE__ */ jsxs("div", { className: "flex shrink-0 items-center gap-2 md:flex-col md:items-end", children: [
                    /* @__PURE__ */ jsx("span", { className: "rounded-full border bg-muted/50 px-2.5 py-0.5 text-xs", children: job.type || details.jobType || "CDD" }),
          ...
          sites.length > 0 && /* @__PURE__ */ jsx("div", { className: "mt-4 text-center", children: /* @__PURE__ */ jsxs("span", { className: "text-xs text-muted-foreground", children: [
            sites.length,
            " offres affich\xE9es"
          ] }) })
        ]
...
// .probe_inside/ssr_entry.mjs
var qc = new QueryClient({ defaultOptions: { queries: { retry: false, gcTime: 0 } } });
try {
  const html = rts(
    el(
      QueryClientProvider,
